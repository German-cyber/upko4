const ProgressBar = ({ progress = 0 }) => {
  const limitedProgress = Math.min(progress, 99);

  return (
    <div className="progress-bar_wrappers">
      <div
        className="delivery-car"
        style={{
          left: limitedProgress === 0 
            ? `calc(${limitedProgress}% - 11px)` 
            : `calc(${limitedProgress}% - 22px)`
        }}
      >
        <svg
        ...
        >
          <path
            fill={limitedProgress < 99 ? "#eb001b" : "#428445"}
      ...
      <div className="progress-bar_wrapper">
        <div className="progress-bar" style={{ width: `${limitedProgress}%` }} />
      </div>
    </div>
  );
};
